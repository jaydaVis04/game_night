...
async function fixture(t, options = {}) {
  ...
  let cookie = '';
  const request = async (url, body, { method = body === undefined ? 'GET' : 'POST', admin = true, headers = {} } = {}) => {
    const response = await fetch(base + url, {
      method,
      headers: { 'X-Victory-Request': '1', ...(body === undefined ? {} : { 'Content-Type': 'application/json' }), ...(admin && cookie ? { Cookie: cookie } : {}), ...headers },
  ...
  const setup = () => request('/api/auth/setup', { username: 'Host', password: 'a-strong-local-password', setupCode: 'test-setup-secret' });
  t.after(async () => { await application.close(); await rm(dataDir, { recursive: true, force: true }); });
  return {
...
test('foundation: persistent roster, seven games, win totals, truthful ties, and reversible history', async t => {
  ...
  assert.equal((await f.request('/api/wins', { playerId: alice.id, gameId: 'coup', requestId: randomUUID() })).status, 409);
  ...
  assert.deepEqual(state.players.map(p => [p.name, p.totalWins, p.rank]), [['Alice', 1, 1], ['Bob', 1, 1], ['Charlie', 0, 3]]);
  ...
  assert.ok((await f.request('/api/history')).body.wins.find(w => w.id === first.body.win.id).reversedAt);
  ...
  assert.equal(f.application.db.prepare('SELECT SUM(total_wins) AS total FROM players').get().total,
    f.application.db.prepare('SELECT COUNT(*) AS total FROM wins WHERE reversed_at IS NULL').get().total);
});
...
test('authorization: setup secret, cookie sessions, multiple hosts, CSRF, safe input, and expired sessions', async t => {
  ...
  assert.equal((await f.request('/api/auth/setup', { username: 'Host', password: 'a-strong-local-password', setupCode: 'wrong' })).status, 403);
  ...
  assert.equal((await f.request('/api/players', { name: 'Bad' }, { headers: { Origin: 'https://malicious.example' } })).status, 403);
  assert.equal((await f.request('/api/players', { name: 'Bad' }, { headers: { 'X-Victory-Request': '0' } })).status, 403);
  assert.equal((await f.request('/api/players', { name: '\u0000' })).status, 400);
  assert.equal((await f.request('/api/admins', { username: 'Second Host', password: 'another-good-password' })).status, 201);
  ...
  assert.equal((await f.request('/api/auth/login', { username: 'second host', password: 'another-good-password' })).status, 200);
  assert.equal((await f.request('/api/auth/login', { username: 'Host', password: 'incorrect-password' })).status, 401);
...
test('night lifecycle: leader clears on absence, game change, setting off, win, and new night', async t => {
  ...
  const pending = (await f.request(`/api/join/${previousNight.code}`, { name: 'Guest', requestId: randomUUID() }, { admin: false })).body.request;
  ...
  assert.equal((await f.request(`/api/join-status/${pending.id}?token=${pending.token}`)).body.request.status, 'expired');
});
...
test('join flow: private status, exact and fuzzy matches, explicit duplicate creation, deny, and repeat approval', async t => {
  ...
  assert.equal((await f.request(`/api/join/${code}`, op, { admin: false })).body.request.id, submitted.id);
  ...
  assert.equal((await f.request(`/api/join-status/${submitted.id}?token=${submitted.token}`)).body.request.status, 'pending');
  const queue = (await f.request('/api/joins')).body.requests;
  assert.equal(queue[0].matches[0].id, player.id);
  assert.equal((await f.request(`/api/joins/${submitted.id}/resolve`, { action: 'approve' })).status, 409);
  assert.equal((await f.request(`/api/joins/${submitted.id}/resolve`, { action: 'approve', playerId: player.id })).status, 200);
  assert.equal((await f.request(`/api/joins/${submitted.id}/resolve`, { action: 'approve', playerId: player.id })).status, 200);
  assert.equal((await f.request('/api/state')).body.players.length, 1);
  assert.equal((await f.request(`/api/join-status/${submitted.id}?token=${submitted.token}`)).body.request.status, 'approved');
  const duplicate = (await f.request(`/api/join/${code}`, { name: 'Alice', requestId: randomUUID() }, { admin: false })).body.request;
  ...
  const denied = (await f.request(`/api/join/${code}`, { name: 'Denied guest', requestId: randomUUID() }, { admin: false })).body.request;
  await f.request(`/api/joins/${denied.id}/resolve`, { action: 'deny' });
  assert.equal((await f.request(`/api/join-status/${denied.id}?token=${denied.token}`)).body.request.status, 'denied');
  assert.equal((await f.request(`/api/joins/${denied.id}/resolve`, { action: 'approve' })).status, 409);
});
...
test('game configuration: validated data, persistent admin edits, additional games without build, archive preserves wins', async t => {
  ...
  assert.equal((await f.request('/api/games', { ...base, id: 'custom-game', name: 'Our Custom Game' })).status, 201);
  assert.equal((await f.request('/api/games', { ...base, id: 'unsafe', palette: { ...base.palette, accent: 'url(https://bad)' } })).status, 400);
  const lowContrast = await f.request('/api/games', { ...base, id: 'unreadable', palette: { ...base.palette, text: base.palette.background } });
  assert.equal(lowContrast.status, 400);
  assert.match(lowContrast.body.error, /contrast/);
  assert.equal((await f.request('/api/games/monopoly', { name: 'Family Monopoly' }, { method: 'PATCH' })).status, 200);
  await f.restart();
  assert.equal((await f.request('/api/state')).body.games.find(game => game.id === 'monopoly').name, 'Family Monopoly');
  ...
  await f.request('/api/wins', { playerId: player.id, gameId: 'custom-game', requestId: randomUUID() });
...
test('live updates: public invalidation, exactly one celebration per win, undo, and no replay on reconnect', async t => {
  ...
  assert.ok(events.filter(event => event.type === 'changed').every(event => Object.keys(event).length === 1));
  ...
  fresh.terminate();
  const hostile = new WebSocket(f.base.replace('http:', 'ws:') + '/ws', { origin: 'https://malicious.example' });
