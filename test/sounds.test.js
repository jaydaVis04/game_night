...
test('saved sounds: authentication, bounded files, persistence, playback, selection and removal', async t => {
  ...
  const wave = Buffer.from(encodeWave({ sampleRate: 8000, length: 16000, numberOfChannels: 1, getChannelData: () => new Float32Array(16000).fill(0.1) }, 0, 2));
  const headers = { 'X-Victory-Request': '1', 'Content-Type': 'audio/wav', 'X-Clip-Name': encodeURIComponent('Our victory ♪') };
  assert.equal((await fetch(`${base}/api/sounds`, { method: 'POST', headers, body: wave })).status, 401);
  const setup = await fetch(`${base}/api/auth/setup`, { method: 'POST', headers: { 'X-Victory-Request': '1', 'Content-Type': 'application/json' }, body: JSON.stringify({ username: 'sound-host', password: 'a-valid-local-password', setupCode: 'audio-setup-test' }) });
  assert.equal(setup.status, 201);
  const cookie = setup.headers.get('set-cookie').split(';')[0];
  const authed = { ...headers, Cookie: cookie };
  assert.equal((await fetch(`${base}/api/sounds`, { method: 'POST', headers: authed, body: Buffer.from('<html>not audio</html>') })).status, 400);
  assert.equal((await fetch(`${base}/api/sounds`, { method: 'POST', headers: { ...authed, 'X-Clip-Name': '%' }, body: wave })).status, 400);
  const response = await fetch(`${base}/api/sounds`, { method: 'POST', headers: authed, body: wave });
  ...
  const selected = await fetch(`${base}/api/settings`, { method: 'PATCH', headers: { Cookie: cookie, 'X-Victory-Request': '1', 'Content-Type': 'application/json' }, body: JSON.stringify({ soundId: sound.id }) });
