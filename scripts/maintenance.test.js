...
function run(script, dataDir, args = []) {
  return spawnSync(process.execPath, [join(root, 'scripts', script), ...args], {
    cwd: root, encoding: 'utf8', env: { ...process.env, VICTORY_DATA_DIR: dataDir },
  });
}
...
test('setup is repeatable, preserves records, and seeds exactly seven games', (t) => {
  ...
  db.prepare('INSERT INTO players(id,name,name_key,created_at) VALUES (?,?,?,?)').run('player-fixture', 'Avery', 'avery', new Date().toISOString());
  db.close();
  assert.equal(run('setup.js', dir).status, 0);
  db = openDatabase(dir);
...
test('server lock blocks maintenance and recovers a crashed owner', { timeout: 10000 }, async (t) => {
  const dir = fixture(t);
  ...
  const child = spawn(process.execPath, ['--input-type=module', '-e', `import { acquireServerLock } from ${JSON.stringify(lockUrl)}; acquireServerLock(process.argv[1]); console.log('locked'); setInterval(() => {}, 1000);`, dir], { stdio: ['ignore', 'pipe', 'pipe'] });
...
test('backup snapshots persisted data and referenced audio without changing the source', (t) => {
  ...
  db.prepare('INSERT INTO sounds(id,name,duration,filename,created_at) VALUES (?,?,?,?,?)').run('clip', 'Winner', 1, 'clip.wav', new Date().toISOString());
  db.close();
...
test('missing audio prevents a backup from being marked complete', (t) => {
  ...
  db.prepare('INSERT INTO sounds(id,name,duration,filename,created_at) VALUES (?,?,?,?,?)').run('missing', 'Missing', 1, 'missing.wav', new Date().toISOString());
  db.close();
...
test('desktop and command launchers stay in selected folders and preserve unrelated files', { skip: process.platform === 'win32' }, (t) => {
  const dir = fixture(t);
  const bin = join(dir, 'command folder');
  const desktop = join(dir, 'desktop folder');
  const env = { ...process.env, VICTORY_BIN_DIR: bin, VICTORY_SHORTCUT_DIR: desktop, VICTORY_SKIP_SHORTCUTS: '0' };
  ...
  const desktopFile = join(desktop, process.platform === 'darwin' ? 'Victory Club.command' : 'Victory Club.desktop');
  ...
  assert.equal(spawnSync(process.execPath, args, { env }).status, 0);
  assert.equal(readFileSync(target, 'utf8'), 'Victory Club game-night launcher — user-owned content');
});
