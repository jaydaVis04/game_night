...
function readPassword(label) {
  return new Promise((resolve, reject) => {
    ...
    const onKey = (text, key = {}) => {
      if (key.ctrl && key.name === 'c') return finish(new Error('Canceled. No password was changed.'));
      if (key.name === 'return' || key.name === 'enter') return finish();
      if (key.name === 'backspace') { value = Array.from(value).slice(0, -1).join(''); return; }
      if (key.ctrl || key.meta || ['escape', 'up', 'down', 'left', 'right', 'tab', 'delete', 'home', 'end'].includes(key.name)) return;
...
let db;
try {
  if (!stdin.isTTY || !stdout.isTTY) throw new Error('Open a terminal and run npm run reset-admin. Passwords are entered privately; command-line passwords are not supported.');
  const dataDir = resolveDataDir();
  await access(join(dataDir, 'database.sqlite'));
  release = acquireServerLock(dataDir);
  db = openDatabase(dataDir);
  console.log('Reset an existing host account. This signs every host out.\nYour password will not appear while typing. Press Ctrl+C to cancel.\n');
  ...
  if (Buffer.byteLength(password, 'utf8') < 10 || Buffer.byteLength(password, 'utf8') > 72) throw new Error('Use a password between 10 and 72 UTF-8 bytes. No changes were made.');
