...
export function acquireServerLock(dataDir) {
  ...
  try {
    ...
    lock.exec(
      'PRAGMA journal_mode=DELETE; CREATE TABLE IF NOT EXISTS process_lock (id INTEGER PRIMARY KEY); BEGIN EXCLUSIVE;',
    );
  } catch (error) {
    lock?.close();
    if ([5, 6].includes(error.errcode & 0xff) || /database is (locked|busy)/i.test(error.message)) {
      throw new Error(
        'Victory Club is already using this data folder. Stop the server before running this command.',
      );
    }
    throw error;
  }
