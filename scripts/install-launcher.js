...
if (process.env.VICTORY_SKIP_SHORTCUTS !== '1') {
  ...
  if (process.platform === 'linux' && !process.env.VICTORY_SHORTCUT_DIR) {
    try { desktop = execFileSync('xdg-user-dir', ['DESKTOP'], { encoding: 'utf8' }).trim() || desktop; } catch { /* Standard fallback. */ }
  }
  await mkdir(desktop, { recursive: true });
  if (process.platform === 'darwin') await writeLauncher(join(desktop, 'Victory Club.command'), script);
  else if (process.platform === 'linux') {
    const desktopQuote = (value) => `"${value.replaceAll('\\', '\\\\').replaceAll('"', '\\"').replaceAll('`', '\\`').replaceAll('$', '\\$').replaceAll('%', '%%')}"`;
    await writeLauncher(join(desktop, 'Victory Club.desktop'), `[Desktop Entry]\nType=Application\nVersion=1.0\nName=Victory Club\nComment=${marker}\nExec=/bin/sh ${desktopQuote(join(root, 'scripts', 'launch.sh'))}\nTerminal=true\nCategories=Game;\n`);
    console.log('If your desktop asks, choose “Allow launching” for Victory Club.');
  }
  if (shortCommand) {
    console.log(`Short command: ${join(bin, 'victory-club')}`);
    if (!process.env.PATH?.split(':').includes(bin)) console.log(`To use “victory-club” from any terminal, add ${bin} to your PATH.`);
  }
}
