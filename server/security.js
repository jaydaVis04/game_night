...
export function lanAddresses() {
  return Object.entries(networkInterfaces())
    ...
    .sort(
      (a, b) =>
        Number(/^(utun|tun|docker|bridge|vbox|vmnet)/.test(a.name)) -
        Number(/^(utun|tun|docker|bridge|vbox|vmnet)/.test(b.name)),
    );
}
...
export function sessionMiddleware(db) {
  return (req, res, next) => {
    ...
    const token = (req.headers.cookie ?? '')
      .split(';')
      .map((part) => part.trim())
      .find((part) => part.startsWith('victory_session='))
      ?.slice(16);
    if (token && /^[A-Za-z0-9_-]{43}$/.test(token)) {
      ...
      req.admin =
        ...
        WHERE s.token_hash=? AND s.expires_at > ?`,
          ...
          .get(req.sessionHash, new Date().toISOString()) ?? null;
    }
    next();
  };
}
...
export function createSession(db, req, res, admin) {
  ...
  res.cookie('victory_session', token, {
    httpOnly: true,
    sameSite: 'strict',
    secure: req.secure,
...
export function rateLimit({ max, windowMs }) {
  ...
  return (req, res, next) => {
    ...
    if (++value.count > max) {
      ...
      return next(
        Object.assign(new Error('Too many attempts. Wait a moment and try again.'), {
          status: 429,
        }),
      );
    }
...
export function validatePublicUrl(value) {
  ...
  try {
    ...
    fail(400, 'VICTORY_PUBLIC_URL must be a full HTTP address.');
  }
  if (
