...
export function inspectWave(buffer) {
  ...
  if (buffer.readUInt32LE(4) !== buffer.length - 8 || buffer.readUInt32LE(16) !== 16 ||
      buffer.readUInt16LE(20) !== 1 || ![1, 2].includes(channels) ||
      sampleRate < 8000 || sampleRate > 96000 || buffer.readUInt16LE(34) !== 16 ||
      buffer.readUInt16LE(32) !== blockAlign || buffer.readUInt32LE(28) !== sampleRate * blockAlign ||
      length !== buffer.length - 44 || length % blockAlign !== 0) throw invalid();
...
export function createSoundRouter({ db, dataDir, requireAdmin, broadcast }) {
  ...
  router.get('/', (_req, res) => res.json({ sounds: db.prepare('SELECT * FROM sounds ORDER BY created_at DESC').all().map(soundJson) }));
  router.post('/', raw({ type: 'audio/wav', limit: '4mb' }), async (req, res) => {
    const { duration } = inspectWave(req.body);
    let name;
    try { name = decodeURIComponent(req.get('X-Clip-Name') || '').trim().replace(/\s+/g, ' '); }
    catch { throw Object.assign(new Error('Give your clip a valid name.'), { status: 400 }); }
  ...
  router.delete('/:id', async (req, res) => {
    ...
    try {
      ...
      if (selected && JSON.parse(selected.value) === row.id) {
        db.prepare("UPDATE settings SET value=? WHERE key='soundId'").run(JSON.stringify('default'));
      }
      db.exec('COMMIT');
    } catch (error) { db.exec('ROLLBACK'); throw error; }
    // A failed cleanup leaves only an unreachable file; never roll back the visible state.
    if (row.filename === `${row.id}.wav`) await unlink(join(directory, row.filename)).catch(() => {});
    broadcast({ type: 'changed' });
    res.json({ ok: true });
  });
  return router;
...
export function createMediaRouter({ db, dataDir }) {
  ...
  router.get('/:filename', (req, res, next) => {
    ...
    const row = db.prepare('SELECT filename FROM sounds WHERE filename = ?').get(req.params.filename);
