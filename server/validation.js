...
export function credentials(body) {
  ...
  if (
    ...
    Buffer.byteLength(body.password, 'utf8') < 10 ||
...
export function validateGame(data, existing = {}) {
  ...
  if (typeof id !== 'string' || !/^[a-z0-9][a-z0-9-]{0,59}$/.test(id))
    fail(400, 'Game ID must contain lowercase letters, numbers, or dashes.');
  ...
  if (!merged.palette || typeof merged.palette !== 'object')
    fail(400, 'A complete color palette is required.');
  const palette = {};
  for (const key of PALETTE_KEYS) {
    if (typeof merged.palette[key] !== 'string' || !/^#[a-fA-F0-9]{6}$/.test(merged.palette[key]))
      fail(400, `${key} must be a six-digit hex color.`);
  ...
  for (const [foreground, background] of [
    ...
    if (contrastRatio(palette[foreground], palette[background]) < 4.5) {
      fail(
        400,
        `The ${colorNames[foreground]} and ${colorNames[background]} colors are too similar. Increase their contrast so everyone can read the screen.`,
      );
    }
  }
  return {
    ...
    name,
    tagline,
    themeKey: merged.themeKey,
    motif: merged.motif,
    palette,
    archived: merged.archived === undefined ? false : booleanValue(merged.archived, 'Archived'),
  };
}
