...
export function createApplication({
  ...
  host = '0.0.0.0',
  publicUrl,
  setupCode = secretToken().slice(0, 16),
  ...
  const objectBody = (req, res, next) => {
    if (!req.body || typeof req.body !== 'object' || Array.isArray(req.body))
      return next(Object.assign(new Error('Send a JSON object.'), { status: 400 }));
    next();
  };
  const playerById = (id) => {
  ...
  const activePlayer = (id) => {
    ...
    if (
      player.archived ||
      !db
        .prepare('SELECT 1 FROM attendance WHERE night_id=? AND player_id=? AND active=1')
        .get(getNight(db).id, id)
    ) {
      fail(409, 'Add this player to tonight before continuing.');
    }
    return player;
  };
  const saveAttendance = (playerId, active) =>
    db
      .prepare(
        `INSERT INTO attendance(night_id,player_id,active) VALUES (?,?,?)
    ON CONFLICT(night_id,player_id) DO UPDATE SET active=excluded.active`,
      )
      .run(getNight(db).id, playerId, Number(active));
  ...
  const matchPlayers = (name) =>
    getPlayers(db)
      .map((player) => ({
        id: player.id,
        name: player.name,
        totalWins: player.totalWins,
        similarity: similarity(name, player.name),
      }))
      .filter((player) => player.similarity >= 0.66)
      .sort((a, b) => b.similarity - a.similarity || b.totalWins - a.totalWins)
      .slice(0, 5);
  ...
  app.use((req, res, next) => {
    if (shuttingDown)
      return res
        ...
        .status(503)
        .json({ error: 'Victory Club is shutting down. Reopen it to keep playing.' });
    if (!policy.validHost(req))
      return res.status(403).json({ error: 'Use the local address printed by Victory Club.' });
    if (
      !['GET', 'HEAD', 'OPTIONS'].includes(req.method) &&
      (req.get('X-Victory-Request') !== '1' || !policy.validOrigin(req))
    ) {
  ...
  app.get('/api/auth/me', (req, res) =>
    res.json({ admin: req.admin, setupRequired: setupRequired() }),
  );
  ...
  app.post('/api/auth/setup', authLimit, objectBody, async (req, res) => {
    ...
    if (!equalSecret(req.body.setupCode, setupCode))
      fail(403, 'Check the setup code in the server window.');
    ...
    const admin = transaction(db, () => {
      ...
      db.prepare(
        'INSERT INTO admins(id,username,username_key,password_hash,created_at) VALUES (?,?,?,?,?)',
      ).run(admin.id, admin.username, input.usernameKey, hash, new Date().toISOString());
  ...
  app.post('/api/auth/login', authLimit, objectBody, async (req, res) => {
    ...
    const hash =
      row?.password_hash ?? '$2b$12$5rL8gn5cqFbOCW1KeflMpu9gPJCjTYBK7jnazCbDvezjGeLYyoaMm';
    if (!(await bcrypt.compare(input.password, hash)) || !row)
  ...
  app.get('/api/admins', requireAdmin, (req, res) =>
    res.json({
      admins: db
        .prepare('SELECT id,username,created_at AS createdAt FROM admins ORDER BY created_at')
        .all(),
    }),
  );
  app.post('/api/admins', requireAdmin, authLimit, objectBody, async (req, res) => {
    const input = credentials(req.body);
    if (db.prepare('SELECT id FROM admins WHERE username_key=?').get(input.usernameKey))
  ...
  app.patch('/api/players/:id', requireAdmin, objectBody, (req, res) => {
    ...
    const name =
      req.body.name === undefined ? player.name : textValue(req.body.name, 'Player name');
    const archived =
      req.body.archived === undefined
        ? Boolean(player.archived)
        : booleanValue(req.body.archived, 'Archived');
    transaction(db, () => {
      db.prepare('UPDATE players SET name=?,name_key=?,archived=? WHERE id=?').run(
        name,
        nameKey(name),
        Number(archived),
        player.id,
      ...
      if (archived) {
        ...
        db.prepare('UPDATE nights SET leader_id=NULL WHERE ended_at IS NULL AND leader_id=?').run(
          player.id,
        );
      }
    });
  ...
  app.post('/api/night/game', requireAdmin, objectBody, (req, res) => {
    ...
    if (
      ...
      (typeof gameId !== 'string' ||
        !db.prepare('SELECT id FROM games WHERE id=? AND archived=0').get(gameId))
    )
      fail(400, 'Choose an available game.');
  ...
  app.post('/api/night/leader', requireAdmin, objectBody, (req, res) => {
    if (!getSettings(db).currentlyWinning)
      fail(409, 'Turn on the currently-winning setting first.');
  ...
  app.post('/api/wins', requireAdmin, objectBody, (req, res) => {
    ...
    if (existing) {
      if (existing.player_id !== req.body.playerId || existing.game_id !== req.body.gameId)
        fail(409, 'This request ID was already used for another win.');
      return res.json({ win: getWin(db, existing.id) });
    }
    const night = getNight(db);
    if (!night.gameId || req.body.gameId !== night.gameId)
      fail(409, 'Choose the current game before logging a win.');
    activePlayer(req.body.playerId);
    const id = randomUUID();
    transaction(db, () => {
      db.prepare(
        'INSERT INTO wins(id,player_id,game_id,night_id,admin_id,request_id,created_at) VALUES (?,?,?,?,?,?,?)',
      ).run(
        id,
        req.body.playerId,
        night.gameId,
  ...
  app.post('/api/wins/:id/undo', requireAdmin, (req, res) => {
    ...
    if (!win.reversedAt) {
      transaction(db, () => {
        db.prepare('UPDATE wins SET reversed_at=? WHERE id=?').run(
          new Date().toISOString(),
          win.id,
        );
  ...
  app.get('/api/history', requireAdmin, (req, res) => {
    ...
    if (
      ...
      fail(400, 'Choose a limit from 1–200 and a nonnegative offset.');
    res.json({
      wins: getHistory(db, limit, offset),
      total: db.prepare('SELECT COUNT(*) AS total FROM wins').get().total,
    });
  });
  app.patch('/api/settings', requireAdmin, objectBody, (req, res) => {
    const update = {};
    if (req.body.currentlyWinning !== undefined)
      update.currentlyWinning = booleanValue(req.body.currentlyWinning, 'Currently winning');
    if (req.body.soundId !== undefined) {
      if (
        typeof req.body.soundId !== 'string' ||
        (req.body.soundId !== 'default' &&
          !db.prepare('SELECT id FROM sounds WHERE id=?').get(req.body.soundId))
      )
        fail(400, 'Choose an available sound.');
      update.soundId = req.body.soundId;
    }
    transaction(db, () => {
      for (const [key, value] of Object.entries(update))
        db.prepare('UPDATE settings SET value=? WHERE key=?').run(JSON.stringify(value), key);
      if (update.currentlyWinning === false)
        db.prepare('UPDATE nights SET leader_id=NULL WHERE ended_at IS NULL').run();
    });
    changed();
  ...
  function saveGame(game, isNew) {
    if (isNew)
      db.prepare(
        'INSERT INTO games(id,name,theme_key,tagline,motif,palette,archived) VALUES (?,?,?,?,?,?,?)',
      ).run(
        game.id,
        game.name,
        game.themeKey,
      ...
      db.prepare(
        'UPDATE games SET name=?,theme_key=?,tagline=?,motif=?,palette=?,archived=? WHERE id=?',
      ).run(
        game.name,
        game.themeKey,
        game.tagline,
    ...
    if (game.archived)
      db.prepare(
        'UPDATE nights SET game_id=NULL,leader_id=NULL WHERE ended_at IS NULL AND game_id=?',
      ).run(game.id);
  }
  app.post('/api/games', requireAdmin, objectBody, (req, res) => {
    const game = validateGame(req.body);
    if (db.prepare('SELECT id FROM games WHERE id=?').get(game.id))
      fail(409, 'That game ID already exists.');
  ...
  function joinNight(code) {
    const night = getNight(db);
    if (night.code !== code.toUpperCase())
      fail(404, 'This invitation has ended. Scan the latest code on the host screen.');
    return night;
  }
  app.get('/api/join/:code', (req, res) => {
    ...
    res.json({
      valid: true,
      code: night.code,
      gameName: night.gameId
        ? db.prepare('SELECT name FROM games WHERE id=?').get(night.gameId).name
        : null,
    });
  });
  app.post(
    ...
    (req, res) => {
      ...
      const requestId = requestKey(req.body.requestId);
      const existing = db
        .prepare('SELECT id,name,status,token FROM join_requests WHERE night_id=? AND request_id=?')
        .get(night.id, requestId);
      if (existing) {
      ...
      db.prepare(
        'INSERT INTO join_requests(id,night_id,name,request_id,token,created_at) VALUES (?,?,?,?,?,?)',
  ...
  app.get('/api/join-status/:id', (req, res) => {
    const request = db
      .prepare('SELECT id,name,status,token FROM join_requests WHERE id=?')
      .get(req.params.id);
    if (!request || !equalSecret(req.query.token, request.token))
      fail(404, 'That join request was not found.');
    res.json({ request: { id: request.id, name: request.name, status: request.status } });
  });
  app.get('/api/joins', requireAdmin, (req, res) => {
    const requests = db
      .prepare(
        "SELECT id,name,status,created_at AS createdAt FROM join_requests WHERE night_id=? AND status='pending' ORDER BY created_at",
      )
      .all(getNight(db).id)
      .map((request) => ({ ...request, matches: matchPlayers(request.name) }));
    res.json({ requests });
  });
  app.post('/api/joins/:id/resolve', requireAdmin, objectBody, (req, res) => {
    ...
    if (desired === 'approved') {
      ...
      else if (matchPlayers(request.name).length && req.body.createNew !== true)
        fail(409, 'Choose the matching player or explicitly create a new player.');
    }
    transaction(db, () => {
      if (desired === 'approved') {
      ...
      db.prepare('UPDATE join_requests SET status=?,player_id=? WHERE id=?').run(
        desired,
        playerId,
        request.id,
      );
  ...
  app.get('/api/qr', async (req, res) => {
    const svg = await QRCode.toString(`${getLanUrl()}/join/${getNight(db).code}`, {
      type: 'svg',
      margin: 2,
      errorCorrectionLevel: 'M',
      color: { dark: '#211C36', light: '#FFFFFF' },
    });
  ...
  app.get('/{*path}', (req, res) => {
    if (existsSync(path.join(distDir, 'index.html')))
      ...
      res
        .status(503)
        .type('text')
        .send('Victory Club is not built yet. Run npm run build, then reload.');
  });
  ...
  server.on('upgrade', (req, socket, head) => {
    if (
      ...
      req.url !== '/ws' ||
      !req.headers.origin ||
      !policy.validHost(req) ||
      !policy.validOrigin(req)
    ) {
