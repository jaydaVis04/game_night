...
function Waveform({ buffer, start, end, onStart, onEnd }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (let bar = 0; bar < bars; bar += 1) {
        ...
        for (
          let index = bar * stride;
          index < Math.min(samples.length, (bar + 1) * stride);
          index += Math.max(1, Math.floor(stride / 80))
        )
  ...
  const handleMove = (event, change) => {
    ...
    change(
      Math.max(0, Math.min(duration, ((event.clientX - bounds.left) / bounds.width) * duration)),
    );
  };
  ...
  return (
    <div className="wave-track" ref={track}>
      ...
      {[
        ...
        <button
          key={name}
          type="button"
          className={`wave-handle wave-handle-${name}`}
          role="slider"
          ...
          onKeyDown={(event) => keys(event, value, change)}
        >
  ...
  const loadFile = async (file) => {
    ...
    await run(async () => {
      ...
      try {
        ...
        throw new Error('This browser could not read that audio. Try an MP3, WAV, or M4A file.');
      }
      if (decoded.duration < 0.25 || decoded.duration > 600)
        throw new Error('Choose audio between a quarter-second and 10 minutes long.');
      if (current !== generation.current) return;
      setBuffer(decoded);
      setName(file.name.replace(/\.[^.]+$/, '').slice(0, 60));
      setStart(0);
      setEnd(Math.min(10, decoded.duration));
    });
  };
  const changeStart = (value) => {
    const next = Math.max(0, Math.min(buffer.duration - 0.25, value));
    setStart(next);
    setEnd(Math.min(buffer.duration, Math.max(next + 0.25, Math.min(end, next + 10))));
    stopAudio();
  };
  const changeEnd = (value) => {
    const next = Math.max(0.25, Math.min(buffer.duration, value));
    setEnd(next);
    setStart(Math.max(0, Math.min(next - 0.25, Math.max(start, next - 10))));
    stopAudio();
  };
  const selectSound = (id) =>
    run(async () => {
      await soundRequest('/api/settings', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ soundId: id }),
      });
      await onChange?.();
      onNotice?.('Victory sound updated.');
    });
  const previewSound = (id) =>
    run(async () => {
      await enableAudio();
      await playVictory(id);
    });
  const save = (event) => {
    event.preventDefault();
    run(async () => {
      ...
      await soundRequest('/api/sounds', {
        method: 'POST',
        headers: { 'Content-Type': 'audio/wav', 'X-Clip-Name': encodeURIComponent(name.trim()) },
        body: encodeWave(buffer, start, end),
      });
      setBuffer(null);
      setName('');
      await onChange?.();
      onNotice?.('Clip saved. Pick it below for your next victory.');
    });
  };
  return (
    <section className="soundboard" aria-label="Soundboard">
      <div className="sound-intro">
        <h3>A sound worth winning for.</h3>
        <p>Use our victory chime or make a 10-second moment of your own.</p>
      </div>
      <label className={`sound-upload ${busy ? 'is-busy' : ''}`}>
        <span className="sound-upload-symbol" aria-hidden="true">
          ♫
        </span>
        <span>
          <strong>{busy ? 'Working on your audio…' : 'Choose an audio file'}</strong>
          <small>MP3, WAV, M4A and other formats your browser supports. Up to 30 MB.</small>
        </span>
        <input
          type="file"
          accept="audio/*"
          aria-label="Upload win sound"
          disabled={busy}
          onChange={(event) => {
      ...
      {buffer ? (
        <form className="sound-editor" onSubmit={save}>
          <div className="sound-editor-heading">
            <h4>Find the good part</h4>
            <span>{seconds(end - start)} selected</span>
          </div>
          ...
          <div className="sound-time-fields">
            <label>
              ...
              <input
                type="number"
                min="0"
                max={Number((buffer.duration - 0.25).toFixed(2))}
                step="any"
            ...
            <label>
              ...
              <input
                type="number"
                min="0.25"
                max={Number(buffer.duration.toFixed(2))}
                step="any"
            ...
            <button
              ...
              onClick={() => run(() => previewBuffer(buffer, start, end))}
              disabled={busy}
            >
              ▶ Preview slice
            </button>
          ...
          <div className="sound-editor-actions">
            ...
            <button
              type="submit"
              className="sound-button sound-button-primary"
      ...
      <div className="sound-library" aria-label="Saved win sounds">
        {[
          { id: 'default', name: 'Victory chime', duration: 10 },
          ...sounds.filter((sound) => sound.id !== 'default'),
        ].map((sound) => (
          <div
            className={`sound-item ${selectedSoundId === sound.id ? 'selected' : ''}`}
            key={sound.id}
          >
            <button
              type="button"
            ...
            <div className="sound-description">
              ...
              <span>
                ...
                {sound.id === 'default' ? ' · Built in' : ' · Saved clip'}
              </span>
            </div>
            <button
              type="button"
              className="sound-button sound-select"
              disabled={busy || selectedSoundId === sound.id}
              onClick={() => selectSound(sound.id)}
            >
              {selectedSoundId === sound.id ? '✓ Selected' : 'Use sound'}
            </button>
            {sound.id !== 'default' ? (
              <button
                type="button"
                ...
                disabled={busy}
                onClick={() => setRemoveId(removeId === sound.id ? null : sound.id)}
              >
                ×
              </button>
            ) : null}
            {removeId === sound.id ? (
              <div className="sound-delete-confirm">
                <span>Remove this saved clip?</span>
                <button type="button" className="sound-button" onClick={() => setRemoveId(null)}>
                  Keep it
                </button>
                <button
                  type="button"
                  ...
                  onClick={() =>
                    run(async () => {
                      await soundRequest(`/api/sounds/${sound.id}`, { method: 'DELETE' });
                      setRemoveId(null);
                      await onChange?.();
                      onNotice?.('Clip removed.');
                    })
                  }
                >
                  Remove clip
