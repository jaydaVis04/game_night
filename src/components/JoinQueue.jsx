...
export default function JoinQueue({ requests, onClose, onChange, onNotice }) {
  return (
    <Modal
      ...
      description="A quick hello before they join the table."
      onClose={onClose}
      className="queue-modal"
    >
      {requests.length === 0 ? (
        <EmptyState title="Everyone’s accounted for." icon="check">
          New requests will appear here as your friends scan the QR code.
        </EmptyState>
      ) : (
...
function JoinRequest({ request, onChange, onNotice }) {
  ...
  async function resolve(action) {
    ...
    try {
      ...
      await onChange();
      onNotice(
        action === 'approve' ? `${request.name} is welcome at the table.` : 'Request declined.',
      );
    } catch (err) {
  ...
  return (
    <section className="join-request">
      <div className="join-request-heading">
        <Avatar name={request.name} />
        <div>
          <h3>{request.name}</h3>
          <p>Would like to join tonight</p>
        </div>
      </div>
      {request.matches.length ? (
        <fieldset className="match-options">
          <legend>Could this be someone you know?</legend>
          {request.matches.map((match) => (
            <label key={match.id} className="match-option">
              <input
                ...
                value={match.id}
                checked={selection === match.id}
                onChange={() => setSelection(match.id)}
              />
              <span>
                <strong>{match.name}</strong>
                <small>
                  {match.totalWins} lifetime {match.totalWins === 1 ? 'win' : 'wins'}
                </small>
              </span>
            </label>
          ))}
          <label className="match-option">
            ...
            />
            <span>Create a new player named {request.name}</span>
          </label>
        </fieldset>
      ) : (
        <p className="subtle">A new face! Approving will create their player profile.</p>
      )}
      <FormError>{error}</FormError>
      <div className="queue-actions">
        <button className="button secondary" onClick={() => resolve('deny')} disabled={busy}>
          Decline
        </button>
        <button
        ...
        >
          <Icon name="check" />
          {busy ? 'One moment…' : 'Approve player'}
        </button>
      </div>
    </section>
  );
