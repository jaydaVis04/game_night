...
export function getAudioContext() {
  ...
  if (!AudioContextClass) throw new Error('This browser cannot edit audio. Try a recent Chrome, Firefox, or Safari.');
  ...
  for (let index = 0; index < samples.length; index += 1) {
    ...
    for (let note = 0; note < notes.length; note += 1) {
      ...
      if (elapsed >= 0) {
        ...
        sample += 0.075 * envelope * (Math.sin(2 * Math.PI * notes[note] * elapsed) + 0.18 * Math.sin(4 * Math.PI * notes[note] * elapsed));
      }
    }
...
export async function playVictory(soundId = 'default') {
  const context = getAudioContext();
  if (context.state !== 'running') throw new Error('Enable sound on this screen to hear celebrations.');
  stopAudio();
...
export function encodeWave(buffer, start, end) {
  ...
  if (frames <= 0 || frames > sampleRate * 10 + 1) throw new Error('Select a clip no longer than 10 seconds.');
